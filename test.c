...
// 追加: 線分ABへの最近点
Vector closest_point(const Vector A, const Vector B, const Vector P) {
	const Vector AB = sub(B, A);
	const float ab2 = dot(AB, AB);
	float t_ = dot(sub(P, A), AB) / ab2;
	t_ = t_ < 0 ? 0 : (t_ > 1 ? 1 : t_);
	return add(A, mul(AB, t_));
}
...
Wall* addWall(Vector pos1, Vector pos2) {
	...
	const Wall w = {pos1, pos2, a, b, c, n};
...
void reflect(const Ball before, Ball* after) {
	...
	for(int i = 0; i < WALL_MAX; ++i) {
		...
		// before/after の中心から最近点を取る（貫通対策で両方見る）
		const Vector P_before = closest_point(w.pos1, w.pos2, before.center);
		const Vector P_after  = closest_point(w.pos1, w.pos2, after->center);

		const Vector d_after = sub(after->center, P_after);
		const float dist_after = length(d_after);

		// 非接触ならスキップ
		if(dist_after > after->r) {
			// ただし「高速で貫通」して after では離れているケースがあるので
			// before側が接触していた/跨いだ可能性も軽く見る
			const Vector d_before = sub(before.center, P_before);
			const float dist_before = length(d_before);
			if(dist_before > before.r) continue;
		}

		// 法線: 最近点→中心（壁から外向き）
		Vector n = normalize(d_after);
		...
		// めり込み補正（中心を壁外へ）
		const float penetration = after->r - dist_after;
		if(penetration > 0) {
			after->center = add(after->center, mul(n, penetration));
		}
